...
def test_build_edl_full_rules_fallback_has_required_top_level_keys() -> None:
    edl = build_edl(
        session_id="s1",
        manifest=_manifest(),
        candidates_json=_candidates_json(),
        slots_json=_slots_json(),
        selection=None,
    )
    for key in (
        "version",
...
def test_build_edl_llm_selection_no_fallback() -> None:
    selection = {
        "selected": [
            {
                "candidate_id": "c1",
...
def test_build_edl_without_music_has_null_audio_paths() -> None:
    edl = build_edl(
        session_id="s1",
        manifest=_manifest(with_music=False),
        candidates_json=_candidates_json(),
        slots_json=_slots_json(),
        selection=None,
    )
...
def test_w1_low_framing_quality_from_many_upscaled_clips() -> None:
...
def test_w1_low_framing_quality_from_many_cropped_clips() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def test_w1_no_warning_below_threshold() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def test_w2_low_material_quality_from_relaxed_warnings() -> None:
...
def test_w3_weak_rhythm_from_two_peak_off_beat_clips() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def test_w3_weak_rhythm_from_arc_fallback() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def test_w4_slowmo_duplicates_from_slow_hook_on_low_fps_source() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def test_w4_no_warning_when_hook_source_is_high_fps() -> None:
    from edl_agent.edl import _aggregate_warnings
...
def _sfx_clip(slot, role, src, candidate_id, effect="none", effect_params=None):
    return {
        "slot": slot,
        "role": role,
