...
def _flat_features(n, kp_speed=None, sharpness=None, motion_bg=None,
                    subject_visible=None, bbox=(0.4, 0.2, 0.6, 0.8), stride_hz=10.0):
    ...
    return {
        ...
        "subject_visible": subject_visible if subject_visible is not None else [True] * n,
        "subject_bbox": [bbox] * n,
        "multi_subject": [False] * n,
    }


def test_find_peak_windows_detects_prominent_peak_away_from_edges():
    n = 100  # 10s a 10Hz
    kp = [0.1] * n
    kp[50] = 1.0
...
def test_find_peak_windows_discards_near_scene_cut():
    n = 100
    kp = [0.1] * n
    kp[50] = 1.0
    feats = _flat_features(n, kp_speed=kp)
...
def test_find_peak_windows_discards_low_sharpness():
    n = 100
    kp = [0.1] * n
    kp[50] = 1.0
    sharpness = [0.8] * n
...
def test_find_peak_windows_discards_near_clip_edge():
...
def test_find_calm_windows_picks_longest_two_over_two_seconds():
    n = 300  # 30s a 10Hz
    kp = [0.9] * n
    ...
    assert durations == [3.9, 5.9]  # window = [t_first, t_last] del tramo (n-1 pasos de 0.1s)


def test_find_calm_windows_rejects_uncentered_subject():
    n = 50
    feats = _flat_features(n, kp_speed=[0.1] * n, bbox=(0.0, 0.0, 0.1, 0.2))
    assert find_calm_windows(feats) == []


def test_edge_margin_short_vs_long_clip():
    assert edge_margin_s(3.0) == 0.25
    assert edge_margin_s(5.0) == 0.5


def test_admits_slots_filters_by_window_duration():
    slots = [{"slot": 0, "start_f": 0, "end_f": 30}, {"slot": 1, "start_f": 30, "end_f": 300}]
    assert admits_slots((0.0, 1.5), slots) == [0]
    assert admits_slots((0.0, 12.0), slots) == [0, 1]
    assert admits_slots((0.0, 0.5), slots) == []


def test_score_cv_rewards_action_and_centrality_for_peak():
...
def test_score_cv_rewards_stillness_for_calm():
    still = score_cv(kp_speed=0.05, sharpness=0.9, bbox=(0.4, 0.0, 0.6, 1.0), kind="calm")
    moving = score_cv(kp_speed=0.9, sharpness=0.9, bbox=(0.4, 0.0, 0.6, 1.0), kind="calm")
    assert still > moving


def test_build_image_candidate_admits_all_slots():
