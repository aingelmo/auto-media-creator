...
def test_upsert_rating_persists_per_hook_line(tmp_path):
    ...
    hooks = [
        {
            "angle": "pregunta",
            "hook_line": "Empieza corriendo y acaba remando",
            "good": True,
            "note": "grounded",
        },
        {
            "angle": "contraste",
            "hook_line": "Arranca y acaba igual",
            "good": False,
            "note": "template collapse",
        },
    ]
...
def test_upsert_rating_overwrites_previous_round(tmp_path):
    ...
    upsert_rating(
        path, "sess1", [{"angle": "tu", "hook_line": "a", "good": True, "note": ""}]
    )
    ratings = upsert_rating(
        path,
        "sess1",
