...
def _slots(n_develop: int, hook_f=45, develop_f=40, close_f=60):
    slots = [{"slot": 0, "start_f": 0, "end_f": hook_f, "role": "hook", "beats_rel_f": [0, hook_f // 2]}]
    start = hook_f
    for i in range(n_develop):
        slots.append({
            "slot": i + 1, "start_f": start, "end_f": start + develop_f, "role": "develop",
            "beats_rel_f": [0, develop_f // 2],
        })
        start += develop_f
    slots.append({"slot": n_develop + 1, "start_f": start, "end_f": start + close_f, "role": "close",
...
def _selected(cid, role, rank, exercise="back squat"):
    return {"candidate_id": cid, "role": role, "rank": rank, "exercise": exercise, "reason": "test"}
...
def _build_scenario(n_develop: int, aspect_dims: tuple[int, int], seed: int):
    ...
    candidates_by_id[hook_id] = _candidate(hook_id, "hook.mov", "peak", t_peak=1.0, window=(0.0, 5.0))
    selected.append(_selected(hook_id, "hook", 1))

    close_id = "close0"
    sources_by_src["close.mov"] = _source("close.mov", w, h)
    candidates_by_id[close_id] = _candidate(close_id, "close.mov", "calm", t_peak=5.0, window=(3.0, 8.0))
    selected.append(_selected(close_id, "close", 1))

    exercises = ["back squat", "deadlift", "clean", "snatch", "pull-up", "burpee", "thruster", "jerk"]
    for i in range(n_develop):
        cid = f"dev{i}"
        src = f"dev{i}.mov"
        ...
        bbox = (rng.uniform(0.1, 0.3), rng.uniform(0.05, 0.2), rng.uniform(0.6, 0.9), rng.uniform(0.7, 0.95))
        candidates_by_id[cid] = _candidate(cid, src, "peak", t_peak, window, bbox)
        selected.append(_selected(cid, "develop", i + 1, exercise=exercises[i % len(exercises)]))
...
def test_planner_invariants(aspect_name, w, h, n_develop):
    slots, selected, candidates_by_id, sources_by_src = _build_scenario(n_develop, (w, h), seed=n_develop * 7)
    ...
    if develop_clips and "arc_fallback" not in warnings:
        best_rank_id = min(
            (s for s in selected if s["role"] == "develop"), key=lambda s: s["rank"],
        )["candidate_id"]
        assert develop_clips[-1]["candidate_id"] == best_rank_id


def test_admits_respects_margin():
...
def test_arc_order_best_rank_last():
    for k in range(1, 8):
        order = arc_order(k)
...
def test_compute_in_out_peak_lands_on_configured_beat():
    # #6.3: sin clamp, in_s = t_peak - lead * need_s, con lead = beats_rel_f[peak_beat_index] / d_f.
    slot = {"start_f": 0, "end_f": 60, "beats_rel_f": [0, 20, 40]}  # d_f=60, 2s
    cand = _candidate("c0", "a.mov", "peak", t_peak=6.0, window=(0.0, 12.0))
    timing = compute_in_out(cand, slot, role="develop", speed=1.0, config=DEFAULT_CONFIG)
...
def test_compute_in_out_clamped_reports_peak_off_beat():
...
def test_missing_hook_raises():
    ...
    sources_by_src = {"a.mov": _source("a.mov", 1920, 1080), "b.mov": _source("b.mov", 1920, 1080)}
