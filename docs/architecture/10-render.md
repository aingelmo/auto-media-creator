...
- **Outro C0** (cola del close con `outro_frames`, `render.finish_graph`): el logo entra como segundo input (`-i {logo}`; si además hay watermark, `[1:v]split=2` lo comparte) + `[main]boxblur,eq(enable='gte(n,N)');[main][logo]overlay(enable='gte(n,N)', fade alfa propio desde `N/30`);drawtext(handle, enable='gte(n,N)')` con `-frames:v n_frames` (micro-outro 24f/0.8s: logo + handle en el núcleo seguro, sin CTA ni segunda línea; la cabeza del clip no se toca). Preview y final usan el mismo grafo con tamaños escalados, así que R1/R2 aplican igual. El segmento legacy `effect == end_card` (`render_end_card_segment`, lienzo sólido `color=c={bg}` + `handle`/`line`) se conserva solo para re-renderizar EDLs v5 antiguas.
