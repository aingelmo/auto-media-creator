...
- `{color_fix}`: si el clip trae `color_fix` (§6.7), `eq=brightness={brightness},` (sólo luma); si no, vacío. Igual en preview y final (`render_profile.color_fix_filter_template`).
- `{text}` (`render/_common.py:hook_text_filter`): si `effect_params.text` existe, escribe un sidecar `.ass` (`HOOK_ASS_TEMPLATE`, estilo `Hook`: Montserrat ExtraBold, `PlayResX/Y=1080x1920` para que libass escale outline/sombra/tamaño automáticamente a preview o final) y devuelve `ass=filename='{ass_path}':fontsdir='{FONTS_DIR}',`; si no, vacío. Sustituye al antiguo `drawtext` (sin soporte de sombra difusa ni animación de escala en ffmpeg 5.1). El texto en mayúsculas, envuelto a ≤2 líneas por `planner.effects.layout_hook_line`, entra sin fade (pop-in de escala vía `\t()`) y sale con `\fad(0,{fade_ms})`. La ruta del `.ass` y `fontsdir` se escapan con `_drawtext_escape` (mismo escapado de dos niveles que usaban los valores de `drawtext=`). Va después de `{color_fix}` para que la corrección no tiña el blanco. Igual en preview y final (`render_profile.hook_ass_template`, `hook_text_font_sha256`).
...
- **End card C0** (`effect == end_card`, `render_end_card_segment`): inputs `-ss {tail_in_s} -t {dur+0.5} -i {close_src} -i {logo}` (imagen: `-loop 1 -i {close_src} -i {logo}`) + `[0:v]scale={tw}:{th}:force_original_aspect_ratio=increase,crop={tw}:{th},boxblur={blur_radius}:{blur_power},eq=brightness={dim},setsar=1[bg];[1:v]scale={lw}:-1[lg];[bg][lg]overlay=(W-w)/2:(H-h)/2-{1.5·ts}[v1];[v1]drawtext(handle),fade=t=in:st=0:d=0.25,setsar=1,format=yuv420p[v]` con `-frames:v n_frames` (micro-outro 24f/0.8s: logo + handle en el núcleo seguro, sin CTA ni segunda línea). Sin watermark sobre la card. Preview y final usan el mismo grafo con tamaños escalados, así que R1/R2 aplican igual. El grafo legacy de lienzo sólido (`color=c={bg}` + `handle`/`line`) se conserva solo para EDLs antiguas sin `logo_src`.
