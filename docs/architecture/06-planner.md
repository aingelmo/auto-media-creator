...
- Entrada: `sessions/<name>/brand/brand.json` + `brand/logo.png` (los escribe el formulario web; una marca por sesión, sin default a nivel de repo porque hay varios negocios). `brand.json`: `logo` (relativo a la sesión, por defecto `brand/logo.png`), y opcionales `handle`, `bg` (`#111111`), `fg` (`#FFFFFF`), `font` (`hook_text_font`). `session.planner.load_brand` añade `logo_sha256`, `logo_w`, `logo_h`; si falta el JSON o el PNG, la capa está apagada sin error.
...
- **End card C0** (`config.end_card`, `end_card_frames = 24`): `build_clips._split_end_card` quita al close sus últimos `k = min(end_card_frames, n_frames_close − 30)` frames (el close es 1.0x: `out_s = in_s + n_frames/30`) y añade un clip sintético `role = end_card`, `effect = end_card`, con el mismo `src`/`type` del close y `in_s`/`out_s` = su cola (`old_out_s − k/30`, `old_out_s`; `0`, `k/30` si el close es imagen) y `effect_params = {fg, font, handle, logo_src, logo_w: 360, logo_h (por aspecto del PNG), text_size: 64, blur_radius, blur_power, dim: -0.3}` (micro-outro sin CTA ni segunda línea: la dirección vive en la bio). `duration_f` y el corte de música no cambian. Si `k < 15` no hay card y se avisa `end_card_skipped`.
- Invariantes: el slot sintético `{slot: close+1, role: end_card, start_f, end_f}` se añade solo a la lista en memoria que recibe `assert_invariants` (`slots.json` no cambia); P9 exime a `end_card` (mínimo 15 frames); P3 se salta para `end_card` (su cola hereda la ventana del close y su `candidate_id` sintético no está en `candidates_by_id`); P7 exime a `end_card` (su cola es contigua al close por diseño, sin el margen de 0.25s). `apply_color_match` ignora el end card.
