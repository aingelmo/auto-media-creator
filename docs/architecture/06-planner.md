...
- **Punch-in en cada corte de develop** (`config.punch_in`, por defecto true): `effect_params` añade `punch_frames` (5), `punch_zoom` (1.06) — un zoom 1.0 → `punch_zoom` sobre `punch_frames` frames de salida al inicio del clip, luego se mantiene. Solo `role == "develop"`, solo vídeo (las imágenes ya llevan Ken Burns).
- **Flash blanco en el beat pico del hook** (`config.hook_flash`, por defecto true): `effect_params` añade `flash_frame` (= `peak_f`, el frame de salida del beat pico que devuelve `compute_in_out`), `flash_frames` (2) — frame completo blanco durante esos frames. Solo `role == "hook"`.
