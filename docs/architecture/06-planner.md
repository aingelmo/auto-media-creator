...
- **End card C0** (`config.end_card`, `end_card_frames = 24`): `build_clips._apply_outro` anota el close con `effect_params.outro_* = {outro_frames: min(end_card_frames, n_frames), outro_fg, outro_font, outro_handle, outro_logo_src, outro_logo_w: 360, outro_logo_h (por aspecto del PNG), outro_text_size: 64, outro_blur_radius, outro_blur_power, outro_dim: -0.3}` (micro-outro sin CTA ni segunda línea: la dirección vive en la bio). No se roba ningún frame del timeline: el close conserva todos sus frames y su cola hace de tarjeta (fondo difuminado/oscurecido + logo + handle). P9 garantiza close ≥ 30 frames, así que la cola de 24 siempre cabe y la tarjeta nunca se salta. `duration_f` y el corte de música no cambian.
- Invariantes: un clip por slot, sin slots sintéticos (`slots.json` no cambia); P1-P9 aplican a todos los clips por igual. `apply_color_match` ignora el tratamiento outro (solo mide el contenido base del close).
