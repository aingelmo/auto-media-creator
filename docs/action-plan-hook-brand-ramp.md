...
> Superseded again 2026-09-18: the overlay no longer renders via `drawtext`/`_drawtext_escape`. It's now a libass `.ass` sidecar (`render/_common.py:hook_text_filter`, `HOOK_ASS_TEMPLATE`) with bundled Montserrat ExtraBold, all-caps text wrapped to ≤2 lines (`planner/effects.py:layout_hook_line`), thick outline + soft shadow, a 300 ms scale pop-in (no fade-in) and a 200 ms fade-out. See `docs/architecture/10-render.md` §6.6.
