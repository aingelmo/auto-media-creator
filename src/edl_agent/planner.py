...
def place_develop_arc(
    ...
    }  # taken ya viene ordenado por rank
    order = arc_order(k)
    ...
    for _ in range(3):
        ...
        if not dev_violations:
            # violacion solo contra hook/close: no se puede arreglar
            # intercambiando develops
            break
        i = dev_violations[0]
...
def assign_slots(
    ...
    if hook is None:
        msg = "no admissible hook candidate (fallback de reglas fuera de alcance)"
    ...
    if close is None:
        msg = "no admissible close candidate (fallback de reglas fuera de alcance)"
        raise PlannerError(msg)
...
def _assert_invariants(
    ...
    if len(clips) != n_slots or [c["slot"] for c in clips] != [
        ...
        msg = "P1: clips no cubren los slots 1:1 en orden"
        raise PlannerError(msg)

    # P2
    if len({c["candidate_id"] for c in clips}) != len(clips):
        msg = "P2: candidate_id repetido entre clips"
        raise PlannerError(msg)
    ...
    for c in clips:
        ...
        if not (
            ...
            msg = f"P8: crop fuera de [0,1] en slot {c['slot']}"
            raise PlannerError(msg)
        px = c["crop_px"]
        if not (
            px["x"] >= 0
            ...
            msg = f"P8: crop_px fuera de W x H en slot {c['slot']}"
            raise PlannerError(msg)
        if px["w"] % 2 != 0 or px["h"] % 2 != 0:
            msg = f"P8: crop_px impar en slot {c['slot']}"
            raise PlannerError(msg)
        if c["layout"] == "crop" and abs(px["w"] / px["h"] - 9 / 16) * px["h"] > 2:
            msg = f"P8: crop_px no respeta 9:16 en slot {c['slot']}"
            raise PlannerError(msg)

        # P9
        if c["n_frames"] < 30:
            msg = f"P9: n_frames < 30 en slot {c['slot']}"
            raise PlannerError(msg)
    ...
    for a, b in itertools.pairwise(clips):
        if a["timeline_end_f"] != b["timeline_start_f"]:
            msg = f"P6: hueco en la timeline entre slots {a['slot']} y {b['slot']}"
            raise PlannerError(msg)
    ...
    for segs in by_src.values():
        ...
        for a, b in itertools.pairwise(ordered):
            if a["out_s"] + 0.25 > b["in_s"] + 1e-6:
                msg = f"P7: solape de la misma fuente {a['src']}"
                raise PlannerError(msg)
